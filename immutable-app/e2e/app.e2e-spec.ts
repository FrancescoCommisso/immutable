...
describe('Starting tests for immutable-app', function() {
  ...
  it('website title should be immutable-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('immutable-app');
    })
  });

  it('network-name should be immutable-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('immutable-network@0.0.1.bna');
    });
  });

  it('navbar-brand should be immutable-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('immutable-app');
    });
  });
